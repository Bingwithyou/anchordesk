...
`docs/demo/` 提供浏览器端检索演示：在浏览器本地运行轻量演示模型
（`bge-small-zh-v1.5`，WebAssembly 推理），检索、距离与拒答判定均为
真实计算；回答生成不在演示范围内。演示模型、门槛与语料差异必须显著标注，
不得冒充真实系统。
...
- 页面可以使用静态 HTML、CSS、图片与本地推理的演示资源。
- 不维护 mock API。
- 交互演示（docs/demo/）只做真实客户端检索与拒答判定，不得用演示模型伪造生成回答，且必须显著标注与真实系统的差异。
- 截图必须来自通过最终验收的本地应用。
